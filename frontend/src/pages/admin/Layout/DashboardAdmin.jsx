...
const DashboardAdmin = () => {
  ...
  return (
    <div className="flex h-screen bg-gray-100 relative overflow-hidden">
      ...
      >
        {/* Logo Area */}
        <div className="p-5 border-b border-indigo-600 flex items-center justify-between md:justify-start gap-3">
          <div className="flex items-center gap-3">
            <div className="bg-white p-2.5 rounded-xl shadow-lg">
              <Package className="w-7 h-7 text-indigo-700" />
            </div>
            {/* Teks Logo: Tampil jika Mobile Menu Buka ATAU Desktop Sidebar Expanded */}
          ...
          <button 
            onClick={() => setIsMobileMenuOpen(false)}
            className="md:hidden text-white hover:bg-indigo-600 p-1 rounded-md"
          >
            <X className="w-6 h-6" />
          </button>
        </div>
        ...
        {/* Menu Navigation */}
        <nav className="flex-1 px-3 py-4 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-indigo-500">
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`
                  ...
                  active:scale-95 outline-none focus:ring-2 focus:ring-indigo-400 focus:ring-offset-2 focus:ring-offset-transparent
                  ${isActive 
                ...
                `}
              >
                <Icon className={`w-5 h-5 flex-shrink-0 ${isActive ? 'text-white' : 'text-indigo-200'}`} />
                
                {/* Label: Tampil di Mobile atau Desktop Expanded */}
        ...
        <div className="hidden md:block px-3 py-4 border-t border-indigo-600">
          <button 
            onClick={() => setIsDesktopSidebarOpen(!isDesktopSidebarOpen)}
            className="w-full flex items-center justify-center gap-3 px-3 py-2.5 rounded-lg hover:bg-indigo-600/50 transition-colors"
          >
        ...
        {/* Mobile Footer User Info (Optional for easier access on mobile) */}
        <div className="md:hidden p-4 border-t border-indigo-600 bg-indigo-800/50">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden h-full">
        ...
        <header className="bg-white shadow-sm border-b border-gray-200 px-4 sm:px-6 py-4 z-10">
          <div className="flex items-center justify-between">
            ...
            {/* Mobile User Avatar Only (Logout ada di sidebar mobile) */}
